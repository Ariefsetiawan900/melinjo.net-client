...
const ProductDetailsSection = (props) => {
  ...
  const handleButtonWA = (product)=>{
    if(isAuthenticate()){
      window.open(`https://api.whatsapp.com/send?phone=62${product}&text=halo`) 
    }
}
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      document.title = `${responseData.Product.pName} | Mlinjo.net`
    // console.log(responseData.Product.pName);
      setTimeout(() => {
        ...
        <div className="grid grid-cols-2 md:grid-cols-12">
          <div className="hidden md:block md:col-span-1 md:flex md:flex-col md:space-y-4 md:mr-2">
            <img
              ...
              className={`${
                count === 0 ? "" : "opacity-25"
              } cursor-pointer w-20 h-20 object-cover object-center rounded-lg`}
                  ...
                  layoutData.inCart.includes(sProduct._id) === true ? (
                    ...
                    <Fragment>
                      {
                        isAuthenticate() ? (
                    <ButtonC
                    onClick={()=>handleButtonWA(sProduct.pPhoneNumber)}
                      // onClick={(e) =>
                      //   addToCart(
                      //     sProduct._id,
                      //     quantitiy,
                    ...
                    >
                      Beli Sekarang
                          ...
                          <Tippy content={<div className="text-center"><h1>Silahkan Login,<br/>sebelum memesan barang</h1></div>}>
                          <div
                          style={{ background: "#303031" }}
                          // disabled={true}
                          
                          className="px-4 py-2 text-white opacity-50 cursor-not-allowed text-center uppercase rounded-md"
                        >
                          Beli Sekarang
                        </div>
                        </Tippy>
